:- module(prime_gen, 
    [prime_gen_new/1, prime_gen_new/2, prime_gen_value/2, prime_gen_next/2]).
...
*
*   * prime_gen_value(PrimeGen, P): P is the current prime value of PrimeGen. 
*
*   * prime_gen_next(PrimeGen, PrimeGen1): Iterate the prime number generator
*     PrimeGen. 
*
...
%-------------------------------------------------------------------------------
% A PCG is a number wheel initialized to generate numbers that are not multiple
...
spmg_step_up_to_value(SPMG0, Val, SPMG2) :- 
    spmg_value(SPMG0, Cur), 
    Cur < Val, 
    spmg_step(SPMG0, SPMG1), 
    spmg_step_up_to_value(SPMG1, Val, SPMG2).
...
/**
 * prime_gen_value(PG, P)
 *
 * P is the current prime number of PG.
 */

prime_gen_value(pg0([P|_], _), P).

prime_gen_value(pg(PCG, _), P) :-
    pcg_value(PCG, P).

/**
 * prime_gen_next(PG0, PG1)
 *
 * PG1 is the iterated version of PG0.
 */


prime_gen_next(pg0([_, P1|Ps], Ps0), pg0([P1|Ps], Ps0)).

prime_gen_next(pg0([_], Ps0), pg(PCG, Heap)) :- 
    pcg_new(Ps0, PCG),
    spmg_new(PCG, SPMG), 
    heap_new(SPMG, Heap).

prime_gen_next(pg(PCG0, Heap0), PG1) :-
    Heap0 \= void,
    pcg_step(PCG0, PCG1),
    ...
    ->  
        ...
        PG1 = pg(PCG1, Heap2)
    ;
        % Candidate is a multiple of a prime
        prime_gen_next(pg(PCG1, Heap1), PG1)
    ).
...
heap_iterate_up_to_value(Heap, Val, Heap) :- 
    heap_root(Heap, Root),
    spmg_value(Root, Cur), 
    Val =< Cur.

heap_iterate_up_to_value(Heap0, Val, Heap2) :- 
    heap_root(Heap0, SPMG0),
    spmg_value(SPMG0, Cur), 
    Cur < Val, 
    spmg_step_up_to_value(SPMG0, Val, SPMG1),
    heap_update_root(Heap0, SPMG1, Heap1), 
    heap_iterate_up_to_value(Heap1, Val, Heap2).
...
heap_update_root(heap(_, W, Left, Right), SPMG, heap(SPMG, W, Left, Right)) :-
    spmg_value(SPMG, Val), 
    heap_optional_value(Left, Val, LVal),
    heap_optional_value(Right, Val, RVal), 
    Val =< LVal, 
    Val =< RVal.
...
heap_update_root(heap(_, W, Left, Right), SPMG, heap(Root1, W, Left1, Right)) :-
    ...
    heap_optional_value(Right, Val, RVal), 
    LVal < Val, LVal =< RVal,
    heap_root(Left, Root1), 
    heap_update_root(Left, SPMG, Left1).
...
heap_update_root(heap(_, W, Left, Right), SPMG, heap(Root1, W, Left, Right1)) :-
    ...
    heap_optional_value(Right, Val, RVal), 
    RVal < Val, RVal < LVal,
    heap_root(Right, Root1), 
    heap_update_root(Right, SPMG, Right1).
...
heap_add(heap(Root, Size, Left, Right), SPMG, Heap) :- 
    spmg_value(SPMG, Val),
    spmg_value(Root, HeapVal), 
    Size1 is Size + 1, 
    HeapVal =< Val, 
    heap_size(Left,
...
heap_add(heap(Root, Size, Left, Right), SPMG, Heap) :-
    ...
    Size1 is Size + 1, 
    Val < HeapVal, 
    heap_size(Left, LSize), 
    heap_size(Right, RSize),
    (   LSize =< RSize
    ->  
